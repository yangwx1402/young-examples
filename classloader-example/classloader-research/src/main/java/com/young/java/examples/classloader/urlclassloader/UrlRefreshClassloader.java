...
public class UrlRefreshClassloader extends ClassLoader {
    ...
    private void loadClassByRefresh(File jarFile) throws Exception {
        System.out.println("load jar file -"+jarFile.getPath());
        ...
        for (ClassInfo entry : jarClassResult) {
             ...
             * 没有加载过的类才进行加载
             */
            if (!allClasses.containsKey(entry.getClassName())) {
                loadClassFromClassPath(classname, entry.getClassBytes());
                allClasses.put(classname, jarFile.getName());
            }
        }
    }
    ...
    private Class initClass(String className, byte[] bytes) throws IOException {
        try {
            Class clazz = getSystemClassLoader().loadClass(className);
            return clazz;
     ...
     * @throws IOException
     */
    private Class loadClassFromClassPath(String clazzname, byte[] bytes) throws IOException, ClassNotFoundException {
        Class clazz = null;
